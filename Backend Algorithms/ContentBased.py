...
movies = pd.read_csv(r"""C:\Users\Smarty\Downloads\untitled\100k\u.item""", sep="|", header=None,encoding = "ISO-8859-1")
...
genre = pd.read_csv(r"""C:\Users\Smarty\Downloads\untitled\100k\u.genre""", sep="|", header=None)
...
class cool:
    def collabrative(self):
        time_start = time.time()
        global user_data
        mean = user_data.groupby(['user_id'],as_index=False,sort=False).mean().rename(columns={'rating':'rating_mean'})[['user_id','rating_mean']]
        user_data.groupby(['user_id'], as_index=False, sort=False).mean().rename(columns={'rating': 'rating_mean'})[
            ['user_id', 'rating_mean']]
        user_data = pd.merge(user_data, mean, on='user_id', how='left', sort=False)
        user_data['rating_adjusted'] = user_data['rating'] - user_data['rating_mean']
        ...
        user1_data = user_data[(user_data['user_id'] == int(global_int_user_id))]
        ...
        # print(user1_data)
        i = 1
        distinct_movie = np.unique(user_data['movie_id'])

        for movie in distinct_movie:
            item_user = user_data[(user_data['movie_id'] == int(movie))]
            ...
            for user2 in distinct_users1:
                ...
                user2_data = user_data[(user_data['user_id'] == user2)]
                ...
                user_data_1 = pd.merge(user1_data, user2_data[['rating_adjusted2', 'movie_id', 'user_id2']],
                                        on='movie_id', how='inner', sort=False)
                user_data_1['vector_product'] = (user_data_1['rating_adjusted1'] * user_data_1['rating_adjusted2'])
                user_data_1 = user_data_1.groupby(['user_id1', 'user_id2'], as_index=False, sort=False).sum()
                user_data_1['dot'] = user_data_1['vector_product'] / (user1_val * user2_val)
                user_data_all = user_data_all.append(user_data_1, ignore_index=True)
                j = j + 1
            ...
            user_data_all = user_data_all.head(50)
            # print(user_data_all)
            # print("------------------------------------------")
        ...
        for movie in distinct_movie:
            ...
            User_dot_adj_rating = pd.merge(user_data, user_data_append_movie[['dot', 'user_id2', 'user_id1']],
                                              how='inner', left_on='user_id', right_on='user_id2', sort='False')
            # if j % 200 == 0 :
            ...
            if len(np.unique(User_dot_adj_rating1['user_id'])) >= 1:
                try:
                    User_dot_adj_rating1['rating'] = User_dot_adj_rating1['dot'] * User_dot_adj_rating1[
                        'rating_adjusted']
                ...
                User_dot_adj_rating1 = User_dot_adj_rating1.groupby(['user_id1'], as_index=False, sort=False).sum()[
                    ['user_id1', 'rating', 'dot_abs']]
                User_dot_adj_rating1['Rating'] = int(
                    (User_dot_adj_rating1['rating'] / User_dot_adj_rating1['dot_abs']) + user1_mean)
        ...
        collabrative_movies = {}
        for row in User_dot_adj_rating_all.iterrows():
            # print (row[1][2])
            collabrative_movies[(movies.iloc[row[1][2]][1])] = (movie_count.loc[row[1][2]][0])
        # print('--------------------------',collabrative_movies)
        collabrative_movies = sorted(collabrative_movies.items(), key=operator.itemgetter(1))
        print(collabrative_movies)

        # for row in User_dot_adj_rating_all:

        s1 = pd.merge(final_recommendation_movies, User_dot_adj_rating_all, how='inner', on=['movie_id'])
        s1 = pd.DataFrame(s1.loc[s1['Rating'] >= 3])

        # print (s1)
        mixed = {}
        for row in s1.iterrows():
            # print ('_+++++++++++++++++++',row[1][0])
            mixed[(movies.iloc[row[1][0]][1])] = (movie_count.loc[row[1][0]][0])
        mixed = sorted(mixed.items(), key=operator.itemgetter(1))
        print(mixed)
...
def main():
    ...
    global global_int_user_id,user_data,movie_count
    # global_int_user_id = input("Enter User ID :")
    for i in range(801,900):
        user_data = pd.read_csv(r"""C:\Users\Smarty\Downloads\untitled\100k\u.data""", sep="\t", header=None)
        user_data.columns = ["user_id", "movie_id", "rating", "ts"]
        movie_count = DataFrame(user_data.groupby(['movie_id']).size())

        global_int_user_id=i
        time_start = time.time()
        ...
        # show_movie_names()
        # print (pd.merge(movies, movie_count, how='outer',on='movie_id'))
        # print ("Total number of movies that can be recommended from contentbased :"+ str(len(final_recommendation_movies)))

        content_movies={}
        # print (final_recommendation_movies)
        for row in final_recommendation_movies.iterrows():
            # print((movies.iloc[row[0]][1]))
            # print (type(movie_count.loc[row[0]][0]))
            content_movies[(movies.iloc[row[0]][1])]=([movie_count.loc[row[0]][0]])
            # content_movies[row.loc[0,2]]
        # print('-------------------------------------------------------------------------------------------------------------')
        print ('User Id = ',global_int_user_id)
        # print (content_movies)
        time_end = time.time()
        content_movies = sorted(content_movies.items(), key=lambda e: e[1][0])
        print(content_movies)
        a = cool()
        a.collabrative()
