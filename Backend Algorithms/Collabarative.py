...
class collabrative:
    def met(self):
        ...
        for row in User_dot_adj_rating_all.iterrows() :
            # print (row[1][2])
            collabrative_movies[(CB.movies.iloc[row[1][2]][1])]=(CB.movie_count.loc[row[1][2]][0])
        # print('--------------------------',collabrative_movies)
        collabrative_movies = sorted(collabrative_movies.items(), key=lambda e: e)
        print (collabrative_movies)
        ...
        s1 = CB.pd.DataFrame(s1.loc[s1['Rating']>=3])

        # print (s1)
        mixed={}
        for row in s1.iterrows() :
            # print ('_+++++++++++++++++++',row[1][0])
            mixed[(CB.movies.iloc[row[1][0]][1])] = (CB.movie_count.loc[row[1][0]][0])
        mixed = sorted(mixed.items(), key=lambda e: e)
        print (mixed)
